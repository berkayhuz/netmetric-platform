...
const dealsPagePath = path.resolve(__dirname, "./page.tsx");

describe("deals page table path wiring", () => {
  it("uses the single CrmEntityListPage path without rollout flags", () => {
    const source = fs.readFileSync(dealsPagePath, "utf8");
    ...
    expect(source).toContain('routePath="/deals"');
    expect(source).toContain('actionPath="/deals"');
  });
});
