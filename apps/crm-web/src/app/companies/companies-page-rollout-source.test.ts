...
const companiesPagePath = path.resolve(__dirname, "./page.tsx");

describe("companies page table path wiring", () => {
  it("uses the single CrmEntityListPage path without rollout flags", () => {
    const source = fs.readFileSync(companiesPagePath, "utf8");
    ...
    expect(source).toContain('routePath="/companies"');
    expect(source).toContain('actionPath="/companies"');
  });
});
