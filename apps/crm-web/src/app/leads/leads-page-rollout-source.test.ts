...
const leadsPagePath = path.resolve(__dirname, "./page.tsx");

describe("leads page table path wiring", () => {
  it("uses the single CrmEntityListPage path without rollout flags", () => {
    ...
    expect(source).toContain('routePath="/leads"');
    expect(source).toContain('actionPath="/leads"');
  });
});
