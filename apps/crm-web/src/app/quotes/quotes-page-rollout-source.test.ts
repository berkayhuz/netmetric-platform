...
const quotesPagePath = path.resolve(__dirname, "./page.tsx");

describe("quotes page table path wiring", () => {
  it("uses the single CrmEntityListPage path without rollout flags", () => {
    const source = fs.readFileSync(quotesPagePath, "utf8");
    ...
    expect(source).toContain('routePath="/quotes"');
    expect(source).toContain('actionPath="/quotes"');
  });
});
