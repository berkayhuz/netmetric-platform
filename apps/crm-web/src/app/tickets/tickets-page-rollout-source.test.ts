...
const ticketsPagePath = path.resolve(__dirname, "./page.tsx");

describe("tickets page table path wiring", () => {
  it("uses the single CrmEntityListPage path without rollout flags", () => {
    const source = fs.readFileSync(ticketsPagePath, "utf8");
    ...
    expect(source).toContain('routePath="/tickets"');
    expect(source).toContain('actionPath="/tickets"');
  });
});
