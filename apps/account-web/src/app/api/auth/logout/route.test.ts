...
describe("Account logout proxy", () => {
  it("forwards cookies with an origin accepted by Auth API cookie protection", async () => {
    ...
    const request = new NextRequest("http://localhost:7004/api/auth/logout", {
      method: "POST",
      headers: {
        cookie:
    ...
    expect(headers.get("origin")).toBe("http://localhost:7004");
  ...
  it("clears auth cookies on logout response", async () => {
    process.env.NEXT_PUBLIC_API_BASE_URL = "http://localhost:5030";
    process.env.NEXT_PUBLIC_AUTH_URL = "http://localhost:7002";
    process.env.ACCOUNT_ACCESS_COOKIE_NAME = "__Secure-netmetric-access";
    process.env.ACCOUNT_REFRESH_COOKIE_NAME = "__Secure-netmetric-refresh";
    process.env.ACCOUNT_SESSION_COOKIE_NAME = "__Secure-netmetric-session";
