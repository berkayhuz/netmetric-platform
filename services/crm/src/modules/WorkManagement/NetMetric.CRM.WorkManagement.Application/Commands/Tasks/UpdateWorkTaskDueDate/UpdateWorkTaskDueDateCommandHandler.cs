...
public sealed class UpdateWorkTaskDueDateCommandHandler(IWorkManagementDbContext dbContext) : IRequestHandler<UpdateWorkTaskDueDateCommand, WorkTaskDto?>
{
    public async Task<WorkTaskDto?> Handle(UpdateWorkTaskDueDateCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            task.SetDueDate(request.DueAtUtc);
        }
