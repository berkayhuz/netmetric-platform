// <copyright file="DealManagementOutboxProcessor.cs" company="NetMetric">
...
public sealed class DealManagementOutboxProcessor(
    DealManagementDbContext dbContext,
    IDealManagementOutboxPublisher publisher,
    IOptions<DealManagementOutboxProcessorOptions> options,
    DealManagementOutboxMetrics metrics,
    ILogger<DealManagementOutboxProcessor> logger) : IDealManagementOutboxProcessor
{
    ...
    private async Task<bool> TryProcessMessageAsync(DealManagementOutboxMessage message, CancellationToken cancellationToken)
    {
        var value = options.Value;
        var now = DateTimeOffset.UtcNow;
        ...
        {
            logger.LogDebug("deal outbox message {MessageId} was leased by another worker.", message.Id);
        ...
        try
        {
            ...
            logger.LogInformation(
                "deal outbox message published. MessageId={MessageId} EventName={EventName} RoutingKey={RoutingKey}",
                message.Id,
                message.EventName,
                message.RoutingKey);
    ...
    private async Task MarkFailedAsync(DealManagementOutboxMessage message, Exception exception, CancellationToken cancellationToken)
    {
        var value = options.Value;
        var now = DateTimeOffset.UtcNow;
        ...
        if (nextAttemptCount >= value.MaxRetryCount)
        {
            ...
            logger.LogError(
                exception,
                "deal outbox message dead-lettered. MessageId={MessageId} EventName={EventName} AttemptCount={AttemptCount}",
                message.Id,
                message.EventName,
                nextAttemptCount);
        }
        else
        {
            ...
            logger.LogWarning(
                exception,
                "deal outbox publish failed. MessageId={MessageId} EventName={EventName} AttemptCount={AttemptCount} NextAttemptInSeconds={DelaySeconds}",
                message.Id,
                message.EventName,
                nextAttemptCount,
                delay.TotalSeconds);
