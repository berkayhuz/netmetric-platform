// <copyright file="TicketManagementOutboxProcessor.cs" company="NetMetric">
...
public sealed class TicketManagementOutboxProcessor(
    TicketManagementDbContext dbContext,
    ITicketManagementOutboxPublisher publisher,
    IOptions<TicketManagementOutboxProcessorOptions> options,
    TicketManagementOutboxMetrics metrics,
    ILogger<TicketManagementOutboxProcessor> logger) : ITicketManagementOutboxProcessor
{
    ...
    private async Task<bool> TryProcessMessageAsync(TicketManagementOutboxMessage message, CancellationToken cancellationToken)
    {
        var value = options.Value;
        var now = DateTimeOffset.UtcNow;
        ...
        {
            logger.LogDebug("Ticket outbox message {MessageId} was leased by another worker.", message.Id);
        ...
        try
        {
            ...
            logger.LogInformation(
                "Ticket outbox message published. MessageId={MessageId} EventName={EventName} RoutingKey={RoutingKey}",
                message.Id,
                message.EventName,
                message.RoutingKey);
    ...
    private async Task MarkFailedAsync(TicketManagementOutboxMessage message, Exception exception, CancellationToken cancellationToken)
    {
        var value = options.Value;
        var now = DateTimeOffset.UtcNow;
        ...
        if (nextAttemptCount >= value.MaxRetryCount)
        {
            ...
            logger.LogError(
                exception,
                "Ticket outbox message dead-lettered. MessageId={MessageId} EventName={EventName} AttemptCount={AttemptCount}",
                message.Id,
                message.EventName,
                nextAttemptCount);
        }
        else
        {
            ...
            logger.LogWarning(
                exception,
                "Ticket outbox publish failed. MessageId={MessageId} EventName={EventName} AttemptCount={AttemptCount} NextAttemptInSeconds={DelaySeconds}",
                message.Id,
                message.EventName,
                nextAttemptCount,
                delay.TotalSeconds);
