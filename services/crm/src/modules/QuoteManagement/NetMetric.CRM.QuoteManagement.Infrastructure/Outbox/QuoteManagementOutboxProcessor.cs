// <copyright file="QuoteManagementOutboxProcessor.cs" company="NetMetric">
...
public sealed class QuoteManagementOutboxProcessor(
    QuoteManagementDbContext dbContext,
    IQuoteManagementOutboxPublisher publisher,
    IOptions<QuoteManagementOutboxProcessorOptions> options,
    QuoteManagementOutboxMetrics metrics,
    ILogger<QuoteManagementOutboxProcessor> logger) : IQuoteManagementOutboxProcessor
{
    ...
    private async Task<bool> TryProcessMessageAsync(QuoteManagementOutboxMessage message, CancellationToken cancellationToken)
    {
        var value = options.Value;
        var now = DateTimeOffset.UtcNow;
        ...
        {
            logger.LogDebug("quote outbox message {MessageId} was leased by another worker.", message.Id);
        ...
        try
        {
            ...
            await dbContext.SaveChangesAsync(cancellationToken);
            metrics.PublishedMessage();

            logger.LogInformation(
                "quote outbox message published. MessageId={MessageId} EventName={EventName} RoutingKey={RoutingKey}",
                message.Id,
                message.EventName,
                message.RoutingKey);
    ...
    private async Task MarkFailedAsync(QuoteManagementOutboxMessage message, Exception exception, CancellationToken cancellationToken)
    {
        var value = options.Value;
        var now = DateTimeOffset.UtcNow;
        ...
        if (nextAttemptCount >= value.MaxRetryCount)
        {
            ...
            metrics.DeadLetteredMessage();
            logger.LogError(
                exception,
                "quote outbox message dead-lettered. MessageId={MessageId} EventName={EventName} AttemptCount={AttemptCount}",
                message.Id,
                message.EventName,
                nextAttemptCount);
        }
        else
        {
            ...
            metrics.FailedMessage();
            logger.LogWarning(
                exception,
                "quote outbox publish failed. MessageId={MessageId} EventName={EventName} AttemptCount={AttemptCount} NextAttemptInSeconds={DelaySeconds}",
                message.Id,
                message.EventName,
                nextAttemptCount,
                delay.TotalSeconds);
