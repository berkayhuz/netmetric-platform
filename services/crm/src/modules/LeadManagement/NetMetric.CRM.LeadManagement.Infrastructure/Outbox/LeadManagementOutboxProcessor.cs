// <copyright file="LeadManagementOutboxProcessor.cs" company="NetMetric">
...
public sealed class LeadManagementOutboxProcessor(
    LeadManagementDbContext dbContext,
    ILeadManagementOutboxPublisher publisher,
    IOptions<LeadManagementOutboxProcessorOptions> options,
    LeadManagementOutboxMetrics metrics,
    ILogger<LeadManagementOutboxProcessor> logger) : ILeadManagementOutboxProcessor
{
    ...
    private async Task<bool> TryProcessMessageAsync(LeadManagementOutboxMessage message, CancellationToken cancellationToken)
    {
        var value = options.Value;
        var now = DateTimeOffset.UtcNow;
        ...
        {
            logger.LogDebug("Lead outbox message {MessageId} was leased by another worker.", message.Id);
        ...
        try
        {
            ...
            logger.LogInformation(
                "Lead outbox message published. MessageId={MessageId} EventName={EventName} RoutingKey={RoutingKey}",
                message.Id,
                message.EventName,
                message.RoutingKey);
    ...
    private async Task MarkFailedAsync(LeadManagementOutboxMessage message, Exception exception, CancellationToken cancellationToken)
    {
        var value = options.Value;
        var now = DateTimeOffset.UtcNow;
        ...
        if (nextAttemptCount >= value.MaxRetryCount)
        {
            ...
            logger.LogError(
                exception,
                "Lead outbox message dead-lettered. MessageId={MessageId} EventName={EventName} AttemptCount={AttemptCount}",
                message.Id,
                message.EventName,
                nextAttemptCount);
        }
        else
        {
            ...
            logger.LogWarning(
                exception,
                "Lead outbox publish failed. MessageId={MessageId} EventName={EventName} AttemptCount={AttemptCount} NextAttemptInSeconds={DelaySeconds}",
                message.Id,
                message.EventName,
                nextAttemptCount,
                delay.TotalSeconds);
