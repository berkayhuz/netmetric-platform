// <copyright file="OpportunityManagementOutboxProcessor.cs" company="NetMetric">
...
public sealed class OpportunityManagementOutboxProcessor(
    OpportunityManagementDbContext dbContext,
    IOpportunityManagementOutboxPublisher publisher,
    IOptions<OpportunityManagementOutboxProcessorOptions> options,
    OpportunityManagementOutboxMetrics metrics,
    ILogger<OpportunityManagementOutboxProcessor> logger) : IOpportunityManagementOutboxProcessor
{
    ...
    private async Task<bool> TryProcessMessageAsync(OpportunityManagementOutboxMessage message, CancellationToken cancellationToken)
    {
        var value = options.Value;
        var now = DateTimeOffset.UtcNow;
        ...
        {
            logger.LogDebug("opportunity outbox message {MessageId} was leased by another worker.", message.Id);
        ...
        try
        {
            ...
            logger.LogInformation(
                "opportunity outbox message published. MessageId={MessageId} EventName={EventName} RoutingKey={RoutingKey}",
                message.Id,
                message.EventName,
                message.RoutingKey);
    ...
    private async Task MarkFailedAsync(OpportunityManagementOutboxMessage message, Exception exception, CancellationToken cancellationToken)
    {
        var value = options.Value;
        var now = DateTimeOffset.UtcNow;
        ...
        if (nextAttemptCount >= value.MaxRetryCount)
        {
            ...
            logger.LogError(
                exception,
                "opportunity outbox message dead-lettered. MessageId={MessageId} EventName={EventName} AttemptCount={AttemptCount}",
                message.Id,
                message.EventName,
                nextAttemptCount);
        }
        else
        {
            ...
            logger.LogWarning(
                exception,
                "opportunity outbox publish failed. MessageId={MessageId} EventName={EventName} AttemptCount={AttemptCount} NextAttemptInSeconds={DelaySeconds}",
                message.Id,
                message.EventName,
                nextAttemptCount,
                delay.TotalSeconds);
