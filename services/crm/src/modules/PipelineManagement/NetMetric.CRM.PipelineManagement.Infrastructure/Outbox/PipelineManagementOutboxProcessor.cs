// <copyright file="PipelineManagementOutboxProcessor.cs" company="NetMetric">
...
public sealed class PipelineManagementOutboxProcessor(
    PipelineManagementDbContext dbContext,
    IPipelineManagementOutboxPublisher publisher,
    IOptions<PipelineManagementOutboxProcessorOptions> options,
    PipelineManagementOutboxMetrics metrics,
    ILogger<PipelineManagementOutboxProcessor> logger) : IPipelineManagementOutboxProcessor
{
    ...
    private async Task<bool> TryProcessMessageAsync(PipelineManagementOutboxMessage message, CancellationToken cancellationToken)
    {
        var value = options.Value;
        var now = DateTimeOffset.UtcNow;
        ...
        {
            logger.LogDebug("Pipeline outbox message {MessageId} was leased by another worker.", message.Id);
        ...
        try
        {
            ...
            logger.LogInformation(
                "Pipeline outbox message published. MessageId={MessageId} EventName={EventName} RoutingKey={RoutingKey}",
                message.Id,
                message.EventName,
                message.RoutingKey);
    ...
    private async Task MarkFailedAsync(PipelineManagementOutboxMessage message, Exception exception, CancellationToken cancellationToken)
    {
        var value = options.Value;
        var now = DateTimeOffset.UtcNow;
        ...
        if (nextAttemptCount >= value.MaxRetryCount)
        {
            ...
            logger.LogError(
                exception,
                "Pipeline outbox message dead-lettered. MessageId={MessageId} EventName={EventName} AttemptCount={AttemptCount}",
                message.Id,
                message.EventName,
                nextAttemptCount);
        }
        else
        {
            ...
            logger.LogWarning(
                exception,
                "Pipeline outbox publish failed. MessageId={MessageId} EventName={EventName} AttemptCount={AttemptCount} NextAttemptInSeconds={DelaySeconds}",
                message.Id,
                message.EventName,
                nextAttemptCount,
                delay.TotalSeconds);
