// <copyright file="SearchIntegrationEventMessageDispatcherTests.cs" company="NetMetric">
...
namespace NetMetric.Search.Worker.UnitTests;
...
public sealed class SearchIntegrationEventMessageDispatcherTests
{
    ...
    public async Task DispatchAsync_WithIndexRoutingKey_ShouldDeserializeAndCallIngestion()
    {
        ...
        var status = await dispatcher.DispatchAsync("search.index.crm", JsonSerializer.Serialize(message, JsonOptions), CancellationToken.None);
    ...
    public async Task DispatchAsync_WithDeleteRoutingKey_ShouldDeserializeAndCallIngestion()
    {
        ...
        var status = await dispatcher.DispatchAsync("search.delete.crm", JsonSerializer.Serialize(message, JsonOptions), CancellationToken.None);
    ...
    public async Task DispatchAsync_WithReindexRoutingKey_ShouldDeserializeAndCallIngestion()
    {
        ...
        var status = await dispatcher.DispatchAsync("search.reindex.crm", JsonSerializer.Serialize(message, JsonOptions), CancellationToken.None);
    ...
    public async Task DispatchAsync_WithUnknownRoutingKey_ShouldReturnUnsupportedAndNotCallIngestion()
    {
        ...
        var status = await dispatcher.DispatchAsync("search.unknown.crm", "{}", CancellationToken.None);
    ...
    public async Task DispatchAsync_WithInvalidJson_ShouldReturnInvalidPayloadAndNotCallIngestion()
    {
        ...
        var status = await dispatcher.DispatchAsync("search.index.crm", "{invalid-json}", CancellationToken.None);
    ...
    public void WorkerAssembly_ShouldNotReferenceSearchApiAssembly()
    {
        ...
        references.Should().NotContain("NetMetric.Search.API");
    }
