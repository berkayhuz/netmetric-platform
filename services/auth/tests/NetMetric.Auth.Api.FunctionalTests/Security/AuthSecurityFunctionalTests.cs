...
public sealed class AuthSecurityFunctionalTests : IAsyncLifetime
{
    ...
    public async Task Logout_Without_Refresh_Context_Should_Clear_Cookies_And_Not_Revoke()
    {
        ...
        cookies.Should().Contain(cookie => cookie.Contains("__Secure-netmetric-access", StringComparison.Ordinal));
        cookies.Should().Contain(cookie => cookie.Contains("__Secure-netmetric-refresh", StringComparison.Ordinal));
        cookies.Should().Contain(cookie => cookie.Contains("__Secure-netmetric-session", StringComparison.Ordinal));
        cookies.Should().Contain(cookie =>
            cookie.Contains("__Secure-netmetric-access=", StringComparison.Ordinal) &&
        ...
        cookies.Should().Contain(cookie =>
            cookie.Contains("__Secure-netmetric-refresh=", StringComparison.Ordinal) &&
            cookie.Contains("path=/api/auth", StringComparison.OrdinalIgnoreCase) &&
            cookie.Contains("expires=Thu, 01 Jan 1970 00:00:00 GMT", StringComparison.OrdinalIgnoreCase));
        cookies.Should().Contain(cookie =>
            cookie.Contains("__Secure-netmetric-session=", StringComparison.Ordinal) &&
            cookie.Contains("path=/api/auth", StringComparison.OrdinalIgnoreCase) &&
            cookie.Contains("expires=Thu, 01 Jan 1970 00:00:00 GMT", StringComparison.OrdinalIgnoreCase));
    ...
    public async Task Logout_With_Account_Web_Origin_Should_Revoke_Current_Session_And_Clear_Cookies()
    {
        ...
        request.Headers.Add(
            "Cookie",
            $"__Secure-netmetric-access=access-token; __Secure-netmetric-refresh=refresh-token; __Secure-netmetric-session={sessionId:D}");
