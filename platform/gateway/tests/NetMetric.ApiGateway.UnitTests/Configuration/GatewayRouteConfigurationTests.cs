...
{
    [Theory]
    [InlineData("platform/gateway/src/NetMetric.ApiGateway/appsettings.json")]
    [InlineData("platform/gateway/src/NetMetric.ApiGateway/appsettings.Development.json")]
    ...
    public void Config_Should_Keep_Existing_Core_Api_Routes(string configurationPath)
    {
        ...
        routes.TryGetProperty("account-api-route", out _).Should().BeTrue();
        routes.TryGetProperty("tools-api-route", out _).Should().BeTrue();
        routes.TryGetProperty("crm-api-route", out _).Should().BeTrue();
    }
